...
with(ozone_data, plot(datetime, ozone_ppm))

#The syntax of this command is different from that of ggplot() in that it takes on a #multiple user define inputs. These inputs are reburied top get the same detailed level of #graph the ggplot produces. but can be called with a single call where ggplot dose not. 
...
ozone_ppm_over_time <- ggplot(ozone_data, aes(x = datetime, y = ozone_ppm))
...
ozone_ppm_over_time <- ozone_ppm_over_time +
                        geom_point()
ozone_ppm_over_time
...
*** the 'ggplot()'object we created has no layers assingend with it as there are is no Geom applied to it.***
...
ozone_ppm_over_time <- ggplot(filter(ozone_data, ozone_ppm > 0.070), aes(x = datetime, y = ozone_ppm)) +
  geom_point() +
  theme_bw() +
  labs(x = "Time of Measurement (YYYY-MM-DD HH:MM:SS)",
       y = "Ozone Concentration (ppm)",
       title = "Hourly Ozone Measurements in Fort Collins, CO",
       subtitle = "Measurements over EPA threshold of 0.07ppm")
...
Based on all the data being displayed across the graphs made we can see an abvious trend of more ozone concentrations happening durring the warmer months such as july and august with a general decline towards the later half of the data series. this comes from the tempertires cooling downa nd less being trapped in the atmosphere. another part to mention is a huge spike in early July and this can probably be attributed to the 4th of July with has a large amount of fireworks that cause damage to the Ozone at a significantly increased volume as compared to a typical day.
~~~
...
prop_ozone_exceed <- mean(ozone_data$ozone_ppm > 0.070)

paste0("Proportion Exceeding Limit: ", round(prop_ozone_exceed * 100, 2), "%")
```
...
The most usefull extension that I was able to find for my senior design project was tidyterra as my project has a lot to do with data acquisition and manipulation of ground and crop scans of a corn field at various growing stages. and the ability to stitch all the data togther seamlessly into an elevation map that can be compared to one another would have been one possible way we could have solved our problem.
